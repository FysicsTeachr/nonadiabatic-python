...
extern "C" {
  ...
  /**
   * \brief Compute a generic job, returning the NACME vector.
   *
   * This is a custom function to allow for 'coupling' runs. It bypasses
   * the hard-coded 'gradient' run type of the default function.
   *
   * @param[in]  numqmatoms Number of atoms in the QM region
   * @param[out] nacme Double array to store the NACME vector (user-allocated)
   * @param[out] status Status of execution: 0 for success, 2 for failure.
   **/
//  void tc_compute_job_sync_(const int* numqmatoms, double* nacme, int* status);
  //void tc_compute_job_sync_(const double* qmcoords, const int* numqmatoms, double* nacme, int* status);
