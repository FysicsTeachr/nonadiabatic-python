...
//    options.emplace("run","gradient");
    ...
    //////// If 'run' is not specified in the input file, default to gradient
...
//      (*pb_output) = temp_pb_output;
      ...
      TCPB::Output temp_pb_output = TC->ComputeGradient((*pb_input), (*totenergy), qmgrad, mmgrad);
      delete pb_output; // Clean up old result
      pb_output = new TCPB::Output(temp_pb_output); // Store new result
...
void tc_get_energy_(const int* state, double* energy, int* status) {
  try {
    pb_output->GetEnergy(*energy, *state);
  }
  catch (...) {
    (*status) = 1;
    return;
...
// Replace your tc_compute_job_sync_ with this more flexible version
void coupling_using_previous_mm(const char* runtype, const double* qmcoords, const int* numqmatoms, double* result, int* status) {
  // Update coordinates as before
  pb_input->GetMutablePB().mutable_mol()->mutable_xyz()->Resize(3*(*numqmatoms), 0.0);
  for (int i = 0; i < 3*(*numqmatoms); i++) {
    pb_input->GetMutablePB().mutable_mol()->mutable_xyz()->mutable_data()[i] = qmcoords[i];
  }

  // Set the run type based on the new argument
  if (strcmp(runtype, "gradient") == 0) {
    pb_input->GetMutablePB().set_run(terachem_server::JobInput_RunType::JobInput_RunType_GRADIENT);
  } else if (strcmp(runtype, "coupling") == 0) {
    pb_input->GetMutablePB().set_run(terachem_server::JobInput_RunType::JobInput_RunType_COUPLING);
  } else {
    (*status) = 1; // Unrecognized run type
    return;
  }

  try {
    TCPB::Output temp_pb_output = TC->ComputeJobSync((*pb_input));
    delete pb_output;
    pb_output = new TCPB::Output(temp_pb_output);

    // Extract the result from the correct field based on the run type
    if (strcmp(runtype, "gradient") == 0) {
      pb_output->GetGradient(result);
    } else if (strcmp(runtype, "coupling") == 0) {
      pb_output->GetNACME(result);
    }
  }
  catch (...) {
    (*status) = 2; // Calculation failed
    return;
  }
